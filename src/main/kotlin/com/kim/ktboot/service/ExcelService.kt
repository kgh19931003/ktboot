...
import com.kim.ktboot.form.ProductList
...
class ExcelService() {
    ...
    fun productExcelDownload(data: ListPagination<ProductList>, response: HttpServletResponse, fileName: String) {
        ...
        XSSFWorkbook().use { workbook ->
            ...
            data.contents.forEachIndexed { index, value ->
                val row = sheet.createRow(index + 1)
                row.createCell(0).setCellValue(value.productIdx.toString())
                row.createCell(1).setCellValue(value.productName)
                row.createCell(2).setCellValue(value.productPrice.toString())
                row.createCell(3).setCellValue(value.productCreatedAt)
            }
